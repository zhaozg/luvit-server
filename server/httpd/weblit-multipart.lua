...
local    C,     Cf,    Cg,      Ct =
    lpeg.C,lpeg.Cf,lpeg.Cg,lpeg.Ct
local alpha  = R('az')+R('AZ')+R('09')
...
local header_s   = Ct( header * (crlf*header)^0 )
...
local build = function(boundary)
  ...
  local node        = Ct(line*boundary*crlf*header_s*crlf*crlf*C( (P(1)-crlf*line*boundary)^0 ))
