...
return function (req, res, go)
  ...
  if (not lowerHeaders.server) and serverName then
  ...
  if not lowerHeaders.connection then
    local version = tonumber(req.httpVersion) or 1.0
    if (req.headers.connection and string.lower(req.headers.connection)=='keep-alive')
      or version > 1.0 then
